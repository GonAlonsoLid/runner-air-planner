...
def main() -> None:
    ...
    if not args.model.exists():
        ...
        print(
            "   Entrena primero: PYTHONPATH=src python -m runner_air_planner.ml.train"
        )
        sys.exit(1)
    ...
    print("=" * 80)
    print(
        f"✅ Buen momento para correr: {good_count} de {total_count} ({good_count/total_count:.1%})"
    )
    print(
        f"❌ No recomendado: {total_count - good_count} de {total_count} ({(total_count-good_count)/total_count:.1%})"
    )
    print()
    ...
    if not good_stations.empty:
        ...
        for _, row in best.iterrows():
            ...
            print(
                f"    Probabilidad: {row['prob_good']:.1%} | AQI: {row['air_quality_index']:.1f} | Viento: {row['weather_wind_speed_kmh']:.1f} km/h"
            )
        print()
