...
def fetch_payload(url: str = DATASET_URL, *, timeout: int = 30) -> Any:
    ...
    try:
        ...
        raise MadridAirDownloadError(
            f"Madrid open data API returned {error.response.status_code}"
        ) from error
    ...
    if isinstance(payload, Mapping):
        ...
        if "result" in payload and isinstance(payload["result"], Mapping):
            ...
            if isinstance(records, list):
                return records
    raise MadridAirDownloadError(
        "Unexpected payload structure from Madrid air quality dataset"
    )
...
def normalise_record(record: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    has_hourly_format = any(
        key.startswith("h") and len(key) == 3 and key[1:].isdigit()
    ...
    unit = _coerce_str(
        lowered.get("unidad") or lowered.get("unit") or lowered.get("unidades")
    )
    validity_raw = (
        lowered.get("valido") or lowered.get("validez") or lowered.get("valid")
    )
    is_valid = None
    if validity_raw is not None:
        is_valid = str(validity_raw).strip().lower() in {
            "s",
            "si",
            "sí",
            "true",
...
def persist_measurements(output_path: Path = DEFAULT_OUTPUT) -> DownloadResult:
    ...
    if not frame.empty:
        frame["measurement_time"] = frame["measurement_time"].dt.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download Madrid's real-time air quality dataset."
    )
    parser.add_argument(
        "--output",
        type=Path,
